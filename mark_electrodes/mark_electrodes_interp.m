function h=mark_electrodes_interp(subject_name,dataroute,channel_list,channel_values,varargin)
...
%'*_electrodes.png' : pic of electrodes, background must be black
%'label_norm.mat' : channels' labels

% Version 5 , Chen Song, 2013-10-22
% Stable_version
...
mapfile=[dataroute,subject_name,'_cortex.png'];  
electrodefile=[dataroute,subject_name,'_electrodes.png'];
...
for ii=1:length(signif_chan)
    ...
    for j=1:size(rel_indx,1)
        if true_indx(j,1)<=size(skull_flag,1)&&true_indx(j,2)<=size(skull_flag,2)   %may be out of range due to large diameter
